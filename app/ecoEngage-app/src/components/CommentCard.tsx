...
import {deleteCommentById, updateCommentById} from "../services/comments-service.ts";
...
const CommentCard: React.FC<CommentProps> = ({comment}) => {
    ...
    const handleCommentSaveClick = async () => {
        ...
        try {
            ...
            const updatedComment = await updateCommentById(comment.id, {commentContent: commentContent});

            if (updatedComment) { // This check ensures updatedComment is not null
            ...
            setIsError(true);
            setErrorMessage("Failed to update the comment. You're not authorized to make any updates to this comment.");
            setSnackbarOpen(true);
        }
    ...
    const handleCommentDeleteClick = async () => {
        ...
        try {
            ...
            setIsError(true);
            setErrorMessage("Failed to delete the comment. You're not authorized to make any updates to this comment.");
            setSnackbarOpen(true);
        }
